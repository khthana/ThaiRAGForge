...
# computing the same quantity the published per-entity_type tables compute.
#
# These are a CODE-PATH anchor, not a currency claim. An index rebuild moves the
# data, so they must be re-pointed after one -- and re-pointed only against an
# *independently published* report, never against this script's own output, or
# the anchor stops being evidence:
#   ("bm25", "person")        data/results/bm25_hybrid_entity_type_breakdown.md
#   ("qwen3_0.6b", "program") data/results/embedder_significance_test_by_entity_type_9way.md
_PUBLISHED = {
    ("bm25", "person"): 0.8147,
    ("qwen3_0.6b", "program"): 0.6034,  # rebuild #4, 2026-08-18; was 0.6066
}
_ANCHOR_TOL = 5e-5

# What the PREDICTION was registered against, before rebuild #4 re-OCR'd
# `2566/ครั้งที่ 3`. Kept apart from `_PUBLISHED` and rendered in the report, so
# re-pointing an anchor can never silently re-base the pre-registration: the
# `person` half is unmoved and the `program` half is 0.0032 lower, and a reader
...
def self_checks(bm25_cells, dense_cells, chunkers, embedders):
    ...
    out.append((
        f"S1 BM25 `person` aggregate reproduces the published "
        f"{_PUBLISHED[('bm25', 'person')]:.4f}",
        ok,
    ...
    out.append((
        f"S2 dense `{_PREDICTION_EMBEDDER}` `program` aggregate reproduces the "
        f"published {_PUBLISHED[(_PREDICTION_EMBEDDER, 'program')]:.4f}",
        ok,
...
def render(bm25_cells, dense_cells, chunkers, embedders, ceilings, nq, checks):
    lines = [
        ...
        f"**{_REGISTERED[('bm25', 'person')]:.4f}**, dense `program` "
        f"**{_REGISTERED[(_PREDICTION_EMBEDDER, 'program')]:.4f}**). A one-chunker pilot cannot be scored "
    ...
    lines += [
        ...
        "",
        "**Against the registration.** The prediction was registered before rebuild #4. "
        f"BM25 `person` is unmoved ({_REGISTERED[('bm25', 'person')]:.4f} -> "
        f"{across_chunkers(bm25_cells, '-', 'person'):.4f}); "
        f"`{_PREDICTION_EMBEDDER}` `program` moved "
        f"{_REGISTERED[(_PREDICTION_EMBEDDER, 'program')]:.4f} -> "
        f"{across_chunkers(dense_cells, _PREDICTION_EMBEDDER, 'program'):.4f} "
        f"({across_chunkers(dense_cells, _PREDICTION_EMBEDDER, 'program') - _REGISTERED[(_PREDICTION_EMBEDDER, 'program')]:+.4f}). "
        "S1/S2 anchor on the **current** figures; the registered ones are shown here so "
        "re-pointing an anchor cannot re-base the pre-registration unnoticed.",
        "",
