...
exhaustive/unranked numbers. That recall@1000 answers "can the relevant
resolutions be retrieved at all" -- it does not answer "would a user seeing a
top-10 list actually see them," because entity_lookup returns matches in
arbitrary corpus order. (The figure is deliberately not quoted here: it has
moved with every entity_tags_full rebuild -- 0.9291 before 2026-08-05, 0.9422
after, 0.9449 after the 2026-08-12 match_programs repair -- so read it from
gold_entity_lookup_73det_report.md, which stamps the build it was scored
against.) This script measures the metric a real
...
from run_gold_entity_lookup_eval import index_provenance  # noqa: E402
...
def render_report(
    ...
    lines = [
        ...
        "",
        "Generated by `tools/eval/run_gold_entity_boost_eval.py`.",
        "",
...
def main() -> None:
    ...
    report = render_report(
        per_type, etypes, max_relevant, over_cap, len(query_set),
        index_provenance(Path(args.index_dir)),
    )
