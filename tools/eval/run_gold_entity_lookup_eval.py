...
set, broken out by entity_type, on the entity_tags_full index (semantic +
bge-m3, zero corpus-discovery contamination).

That index has been rebuilt twice since this script was written (2026-08-05 for
the resolution_id fix, 2026-08-12 for the match_programs repair), so its size
and its scores are NOT constants of this script: the chunk count is read from
the parquet footer and the build is stamped into the report from the index's
own manifest. Quote a number from the report, never from this docstring.
...
def index_provenance(index_dir: Path) -> str:
    """Describe the index this report was actually scored against.

    Derived, never hardcoded: the line here read "70,789 chunks" through two
    rebuilds (71,077 after 2026-08-05, 71,073 after 2026-08-12) because it was
    a literal. The row count comes from the parquet footer (no data read) and
    the build stamp from the index's own manifest, so the report names its
    source the way RetrievalResult does -- see the E0 attribution work.
    """
    import pyarrow.parquet as pq

    n_chunks = pq.ParquetFile(index_dir / "chunks.parquet").metadata.num_rows
    built, docset = "unknown", "unknown"
    manifest = index_dir / "manifest.json"
    if manifest.exists():
        import json

        m = json.loads(manifest.read_text(encoding="utf-8"))
        built = m.get("timestamp", built)
        docset = m.get("docset_hash", docset)
    return (
        f"- Index: entity_tags_full/{index_dir.name} (semantic + bge-m3, "
        f"{n_chunks:,} chunks, zero contamination); built {built}, "
        f"docset_hash `{docset}`"
    )
...
def main() -> None:
    ...
    report = render_report(
        scores, per_type, etypes, len(query_set), index_provenance(Path(args.index_dir))
    )
