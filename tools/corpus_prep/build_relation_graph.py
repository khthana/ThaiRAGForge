...
def render_report(raw: dict, graph: dict, checks: list[dict], agree: dict,
    ...
    A("- **ชื่อที่ไม่มีในพจนานุกรมถูกดูดเข้าเพื่อนบ้านที่ใกล้ที่สุด** — `match_programs` "
      ...
      "ซึ่งอธิบายแถว `หลักสูตรแพทยศาสตรบัณฑิต` ทั้งแถว")
    A("")
    A("วัดขอบเขตแล้ว ไม่ได้เดา: ชื่อใน `programs.json` **0 จาก 253** ชื่อที่ match ไป"
      ...
      "เท่านั้น และพจนานุกรม 253 รายการเป็น subset ที่คัดมา")
    A("")
    A("**แก้แล้วทั้งสองครึ่ง (2026-08-11)** — `docs/program-matcher-absorption.md` วัดทั้งคลัง"
      "แล้วพบว่ารูปที่ใหญ่ที่สุดของ absorption คือ **สลับระดับปริญญา** (บัณฑิต ↔ มหาบัณฑิต ↔ "
      "ดุษฎีบัณฑิต) และ **47% ของจุดที่พลาดมีคำตอบที่ถูกอยู่ในพจนานุกรมอยู่แล้ว** — "
      "ตัว matcher ไม่ได้หมดตัวเลือก แต่ *จัดอันดับ* ผิด · ตอนนี้ระดับปริญญาทำหน้าที่ "
      "**กรอง candidate set** แล้วเลือกใหม่ (ต้องตรงทั้งระดับและสาขา) ไม่ใช่ปล่อยผ่าน · "
      "ครึ่งหลัง — **ดูดข้ามสาขาที่ระดับเดียวกัน** ซึ่งตัวกรองระดับปริญญามองไม่เห็นโดยโครงสร้าง "
      "— ปิดวันเดียวกันด้วยการเทียบ **หัวปริญญากับสาขาแยกกัน** แทนที่จะเทียบทั้งสตริงรวด "
      "(ratio คิดบนสตริงที่ต่อกัน ความไม่ตรงในครึ่งเดียวจึงถูกอีกครึ่งกลบ: "
      "`ทันตแพทยศาสตรบัณฑิต` ได้ 0.88 เหนือเส้น 0.82 เพราะส่วนที่ใช้ร่วมกันยาวกว่า)")
    A("")
