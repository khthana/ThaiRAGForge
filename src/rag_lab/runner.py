...
def _discover_paths(config: ExperimentConfig) -> list[str]:
    ...
    silently get ingested as fake resolutions. `is_real_resolution_path`
    gates on the same year+session signal `make_resolution_id` uses to
    choose the real id over its path-fallback, rather than
    iter_corpus_files' relative-to-root gate, since `input_dir` can itself
    point inside a year folder (see dev_smoke.yaml)."""
    all_paths = sorted(Path(config.corpus.input_dir).rglob("*.md"))
    paths = [p for p in all_paths if is_real_resolution_path(p)]
