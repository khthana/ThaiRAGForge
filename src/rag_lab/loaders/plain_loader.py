...
from rag_lab.loaders.common import (
    ...
    parse_path,
    read_text,
    strip_course_comparison_tables,
...
class PlainLoader(BaseLoader):
    ...
    (checkbox grids with no retrieval value -- see strip_mapping_tables) and
    compacting old/new course-comparison tables to `CODE Title` lines (13
    files corpus-wide, cited by 0/106 deterministic gold queries -- see
    strip_course_comparison_tables) -- this is the raw baseline otherwise."""

    def load(self, path: str) -> Resolution:
        raw_text = strip_course_comparison_tables(strip_mapping_tables(read_text(path)))
