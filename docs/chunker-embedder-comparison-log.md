...
## Rebuild #4 จบครบ 40/40 — และสิ่งที่มันตอบได้ซึ่งไม่ได้ตั้งใจถาม (2026-08-17)

**สถานะ: 26 → 40 จาก 40 ในวันเดียว 7 batch ไม่ตายเลย** — `make_residual_rebuild_config.py`
รายงาน `already current: 40  remaining: 0` ซึ่งเป็นกฎเดียวกับที่ `I6` ใช้ (เทียบ manifest
timestamp กับ mtime ล่าสุดของ corpus) จึงเป็นหลักฐานชุดเดียวกับที่ I6 จะรายงาน ไม่ต้องเดินทั้ง
corpus ซ้ำเพื่อยืนยัน

ทุก combo ผ่าน 3 gate เดิม: `Done: 1/1 combos built` · ไม่มี `[error]` / `Traceback` /
`CUDA out of memory` / `RuntimeError` ใน log · manifest ใหม่มี `n_resolutions 2854`

| combo | chunker × embedder | จริง (นาที) | ประเมิน |
|---|---|---|---|
| 20 | sentence × qwen3-0.6B | 24.8 | ~23-26 |
| 21 | fixed_size × e5-large | 21.3 | ~20-25 |
| 22 | fixed_size × e5-small | 4.0 | ~5 |
| 23 | fixed_size × jina_v5 | 22.2 | ~25 |
| 24 | fixed_size × SCT | 6.8 | ~9 |
| 25 | fixed_size × ConGen | 4.4 | ~7 |
| 26 | fixed_size × bge-m3 | 20.9 | ~20 |
| 27 | fixed_size × m2v | 0.8 | ~3 |
| 28 | **fixed_size × Qwen3-4B** | **106.1** | ~105 |
| 29 | fixed_size × qwen3-0.6B | 22.1 | ~22 |
| 30 | fixed_size_wordaware × bge-m3 | 23.1 | ~30 |
| 31 | fixed_size(256) × bge-m3 | 23.5 | ~25 |
| 32 | fixed_size(1024) × bge-m3 | 21.6 | ~19 |
| 33 | **normalized × semantic × bge-m3** | **58.0** | ~55-65 |

**1. สาเหตุการตายปิดจบแล้ว: ไม่ใช่ "4B บนการ์ด 12 GB" แต่เป็น `semantic` × 4B โดยเฉพาะ**
combo 4B ทั้งสี่ตัวผ่านหมด — 01 (`semantic`, ผ่านหลังตายมา 5 ครั้ง), 10 (`recursive`, 107),
19 (`sentence`, 126), 28 (`fixed_size`, **106**) — สามตัวหลังผ่านรวดในวันเดียวโดยไม่มีอาการใด ๆ
ต่างกันตรงที่อีกสาม chunker ตัด chunk ให้เสร็จก่อนแล้วค่อย embed เป็นชุด ส่วน `SemanticChunker`
ต้อง embed **ทุกประโยค** ด้วย embedder ตัวเดียวกันเพื่อหาจุดตัด จึงเป็นโหลด GPU ต่อเนื่องยาวที่สุด
ในโปรเจกต์ ข้อสรุปนี้ต้องรอให้ 4B อีกสามตัวผ่านถึงจะพูดได้ — ก่อนหน้านี้มีแต่ตัวอย่างที่ตายกับ
`semantic` จึงแยก "4B" ออกจาก "`semantic` × 4B" ไม่ได้

**2. แบบจำลอง `fixed_size ≈ 0.97 × recursive` ยืนได้ทุกระดับ** — ที่ e5-large 21.3 vs 22.0,
ที่ qwen3-0.6B 22.1 vs 22.5, และที่ **4B 106.1 vs 107.0** คือใช้ทำนายงาน 1.8 ชั่วโมงได้คลาด
เคลื่อน 1% รวมกับ `sentence ≈ 1.15 × recursive` จากวันก่อน ตอนนี้มีตัวคูณครบทั้งสี่ chunker

**3. การทดลองที่ตกมาเองโดยไม่ได้ตั้งใจ: chunk_size 256/512/1024 ตรึงตัวแปรอื่นหมด**
combo 31/26/32 ต่างกันแค่ `chunk_size` (embedder เดียวกัน chunker เดียวกัน corpus เดียวกัน)
ได้ **23.5 / 20.9 / 21.6 นาที — ต่างกันแค่ ~12% ทั้งที่จำนวน chunk ต่างกันราว 4 เท่า** เป็น
หลักฐานตรงที่สุดที่มีสำหรับ "ต้นทุน embedding ขึ้นกับ token รวม ไม่ใช่จำนวน chunk" และอธิบายได้ว่า
ทำไม 256 ถึงแพงที่สุด: `chunk_overlap` คงที่ที่ 50 จึงกลายเป็นสัดส่วนใหญ่ของ chunk เล็ก ทำให้
token ซ้ำเยอะกว่า

**4. ผมประเมิน combo 33 ผิดครั้งแรก แล้วแก้ก่อนรัน — ด้วยความผิดพลาดแบบเดิมกับสองครั้งก่อน**
บอกไว้ ~35 นาที โดยเอา `fixed_size × bge-m3` เป็นฐาน ทั้งที่ฐานที่ถูกคือ **`semantic` × bge-m3**
(`semantic × qwen3-0.6B` ใช้ 61 นาที) แก้เป็น ~55-65 ก่อนกดรัน **ผลจริง 58.0 นาที** บทเรียนเดิม:
เลือกฐานอ้างอิงจาก **chunker** ก่อนเสมอ แล้วค่อยปรับด้วย embedder

**5. `docset_hash` ของ combo 33 ต่างจากอีก 39 ตัวโดยชอบ และตรวจแล้วไม่ใช่ปัญหา** — 39 ตัว
ได้ `091b7a0ad8a5cfbe` แต่ combo 33 ได้ `574945883e8320d0` เพราะ `_docset_hash` แฮชจาก
`raw_text` **หลังผ่าน loader** (`manifest.py:31`) และ combo 33 เป็นตัวเดียวที่ใช้ loader
`normalized` ยืนยันด้วยการโหลดไฟล์เดียวกัน 3 ไฟล์ด้วยทั้งสอง loader: ข้อความต่างกันจริงทุกไฟล์
(1419→1407, 999→988, 1175→1163 อักขระ) จึงเป็น "ต่างเพราะ loader" ไม่ใช่ "corpus เพี้ยน" —
gate ที่ถูกสำหรับ combo นี้คือ `n_resolutions 2854` ไม่ใช่การเทียบ hash กับกลุ่ม plain

**6. Qdrant stale ครบทั้ง 4 collection แล้ว** (จากเดิม 3) — combo 26 (`fixed_size × bge-m3`,
route `faculty`/`unmatched`) เป็นตัวสุดท้ายที่ยังรอด และ rebuild ไปเมื่อ 16:03 น. ตามแผนคือ
**re-ingest ทีเดียวทั้งสี่ แล้วรัน `qdrant_routed_check.py` รอบเดียว** ไม่ไล่ตามทีละ combo

**ยังไม่ได้ทำอะไรฝั่ง downstream เลย** — persisted results / report / ตารางที่ตีพิมพ์ทุกอันยัง
อธิบาย index ชุดเก่าอยู่ ซึ่งคือสภาพ "ผสม" ที่ย้ายลงมาอีกชั้นหนึ่ง ลำดับที่ต้องทำอยู่ใน CLAUDE.md
ย่อหน้า I6
