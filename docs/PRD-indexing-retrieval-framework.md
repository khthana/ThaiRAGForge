...
**สถานะ (2026-07-02):** Implemented — issues #1–#11 ปิดหมดแล้ว ดู `README.md`
§สถานะปัจจุบัน สำหรับข้อจำกัดที่ตั้งใจเหลือไว้ (APIEmbedder เป็น stub, HybridEmbedder
ไม่ได้สร้าง, SemanticChunker threshold ยังไม่ tune). เอกสารนี้คงไว้ตามแผนเดิม
ไม่แก้ไขตามของจริงที่สร้าง.

**หมายเหตุ (2026-08-07):** เอกสารนี้ยัง **แช่แข็งไว้เป็นแผนเดิม** ตามที่ระบุข้างบน —
อ่านเป็นบันทึกว่า "ตอนเริ่มคิดอะไรไว้" ไม่ใช่สถานะปัจจุบัน สิ่งที่เปลี่ยนไปมากที่สุด
นับจากวันเขียน: คลังข้อมูลโตจาก 1,215 เป็น **2,854** ไฟล์, embedder จาก 4 เป็น
**9 ตัว**, SemanticChunker threshold ถูกแก้ไปแล้ว (fragmentation defect, commit
`e8f4b80`) และงานส่วนใหญ่ของโปรเจกต์ตอนนี้ไม่ได้อยู่ที่ตัวเฟรมเวิร์กแต่อยู่ที่
ชั้นการประเมินผล (`tools/eval/`) ซึ่ง PRD ฉบับนี้ไม่ได้ครอบคลุม สถานะจริงอยู่ที่
`README.md` และ `docs/paper-results-summary.md`; ภาพรวมเชิงเล่าเรื่องอยู่ที่
`docs/project-journey.html`
