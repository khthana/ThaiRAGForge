...
คัดจาก candidate pool (147 program + 1,139 person + 14 faculty-adjunct) เป็น
**37 รายการ** stratified ตาม event_count/hit_count (12 program + 12 person + 13
faculty-adjunct) พร้อม rephrase คำถามให้เป็นธรรมชาติ (หมุนเทมเพลตต่อประเภท ไม่แตะ
`relevant_resolution_ids`) เขียนลง `config/eval/gold_query_set.yaml` แล้วรัน
`tools/eval/run_gold_chunker_eval.py` เทียบ 4 chunker บน e5-large — ดูผลละเอียด
ที่ `docs/chunker-embedder-comparison-log.md` § "Gold query set" สรุปสั้นๆ: ผลไม่ชี้
ทางเดียวเหมือน Silver — `fixed_size` ชนะ program-history, `semantic` ชนะ
person-history, faculty-adjunct-aggregate ติดเพดาน recall@10 ทางคณิตศาสตร์ (relevant
set ใหญ่) แต่ MRR เกือบเต็มทุก chunker
...
- ขยาย gold set เกิน 37 รายการถ้าต้องการ coverage มากขึ้น (candidate pool ยังเหลือ
  อีกเยอะที่ไม่ได้เลือกมา)
