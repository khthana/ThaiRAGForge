...
## 10. ปิดหนี้เล็กสามอย่างที่ค้างจาก § 9 (21 ส.ค. 2569)

### 10.1 `programme_groups` มีผู้เรียกจริงแล้ว — และมันซ่อมข้อบกพร่องที่มีอยู่จริง

§ 9.2 ทิ้งไว้ว่าฟังก์ชันนี้ไม่มีใครเรียกนอกจากเทสต์ของตัวเอง · ผู้เรียกที่ถูกต้องคือ
`build_relation_graph.py`: กราฟ `หลักสูตร → คณะ` เคยคีย์ด้วย **รายการพจนานุกรม** จึง
**นับคู่โคเซ็น 3 คู่เป็น 6 และซอยหลักฐานออกเป็นสองกอง** ซึ่งไม่ใช่แค่เรื่องตัวส่วน:
ครึ่งหนึ่งของคู่แมคคาทรอนิกส์มีพยานเดียวจึงตกเป็น `ambiguous` ขณะที่ฝาแฝดของมัน
`resolved` ด้วย 4 โหวต**ที่คณะเดียวกัน**

`build_graph` เดิน `programme_groups()` แล้วรวมโหวตของกลุ่มไว้ที่ชื่อแรกตามลำดับ
พจนานุกรม (นิ่ง ไม่เลื่อนระหว่างรัน) ชื่อที่เหลือเก็บไว้ในฟิลด์ `aka` ·
**182 / 57 / 14 จาก 253 → 180 / 56 / 14 จาก 250** บนหลักฐานชุดเดิมทุกไบต์ (cache 11 ส.ค.)
— **เป็นการเปลี่ยน*นิยาม* ไม่ใช่การวัดใหม่ ห้ามอ่านคู่กันเป็น drift**

สองผลต่อ self-check: **`S2` ต้องเดินตามเรื่องที่มันตรวจ** (เดิม gate ว่า
`len(records) == 253` ซึ่งจะ **FAIL บนกราฟที่ถูกต้อง**) และเพิ่ม **`S5`** ที่ gate ว่าทั้ง
253 ชื่อยังเข้าถึงได้ — ความเสี่ยงของการจัดกลุ่มไม่ใช่การรวมผิด (`programme_groups` มีเทสต์
อยู่แล้ว) แต่คือ**ชื่อหายเงียบๆ** ซึ่งจะย่อทุกตัวส่วนในรายงานโดยที่เช็คอื่นยังผ่านหมด ·
การตรวจไขว้ใน § 3 ของรายงานยัง**นับรายชื่อ ไม่ใช่รายหลักสูตร** โดยตั้งใจ

### 10.2 `anchor_status` 414 / 66 / 198 — ยืนยันแล้ว และการยืนยันเจอของแถม

CLAUDE.md กำกับตัวเลขนี้ว่า **UNVERIFIED** ตั้งแต่ 20 ส.ค. เพราะ `gold_candidates.json`
บนดิสก์ไม่มีคีย์ `anchor_status` เลย · นั่นเป็นเรื่อง **provenance ไม่ใช่ความขัดแย้ง**:
ไฟล์ลงวันที่ 25 ก.ค. ส่วนฟิลด์นี้เพิ่มเข้ามา 9 ส.ค. มันจึง*มีไม่ได้* · รันสคริปต์ใหม่ลง
`--output-dir` ชั่วคราว (~4 นาที; **ไม่ทับ**ของ 25 ก.ค. ซึ่งเป็น provenance ของ gold set
ที่ตีพิมพ์) ได้ `ok 414 / ambiguous 66 / no_name_evidence 198` จาก 678 — **ตรงเป๊ะ**

**แต่การรันใหม่เจอสิ่งที่การตรวจตัวเลขเจอไม่ได้**: pool ฝั่งบุคคลขยับ **1,139 → 1,119**
และ **1 ใน 30 person gold entity ที่ตีพิมพ์ไม่ถูกเสนอเป็น candidate อีกแล้ว** —
`ดร.กลกรณ์ วงศ์ภาคิกะเสรี` เคยมี `hit_count 2` ตอน ก.ค. วันนี้เหลือ 1 เพราะเอกสารสองฉบับ
ของมันสะกดนามสกุลต่างกันหนึ่งตัวอักษรหลัง re-OCR (`วงศ์ภา**คิ**กะเสรี` กับ
`วงศ์ภา**ติ**กะเสรี`) จึงไม่มีสะกดไหนถึง `min_person_hits=2`

**qrels ยังถูกต้องและไม่มีการแก้อะไร**: ทั้งสอง resolution เกี่ยวกับบุคคลคนนั้นจริง และ
สตริงใน gold คือสิ่งที่คลังมีตอนคัดเข้า · สิ่งที่มันเป็นคือตัวอย่างรูปธรรมของ pooling bias
ใน `docs/eval-validity-threats.md` § 2 (เอกสารที่เกี่ยวข้องแต่สะกดต่างคือ*ข้อยาก*
ไม่ใช่*ป้ายผิด*) และเป็นบทเรียนว่า **candidate pool คือหลักฐานที่มีวันที่ ไม่ใช่คุณสมบัติ
ถาวรของคลัง** — re-derive หลัง re-OCR แล้วจะไม่ได้ของเดิม · การแก้สตริงจะขยับตัวเลข
person ที่ตีพิมพ์ทุกตัวเพื่อคำถามเดียว จึงไม่ทำ

### 10.3 typo `สาขาวิชาแมคคาทรอนิกส์` — ไม่ใช่ของเรา จึงปิดเรื่อง ไม่ใช่เข้าคิว

§ 9.3 บันทึกไว้ว่าเป็น "typo ใน manifest หนึ่งไฟล์" · **ผิด** — อ่านตัวเอกสารแล้ว
`2566/ครั้งที่ 9` **พิมพ์บรรทัดวาระของตัวเองแบบนั้นจริง** (ส่วนอีก 4 ชื่อของหลักสูตร
เดียวกันทั่วคลังมี `วิศวกรรม` ครบ) manifest จึงเป็นการถอดความที่ **ซื่อตรง** และ typo
เป็นของต้นฉบับ

การแก้ title จะทำให้ metadata **ขัดกับเอกสารที่มันชี้ไป** — ตรงข้ามกับการซ่อม mispairing
เมื่อ 8 ส.ค. ที่ซ่อม title ที่ขัดกับไฟล์ของตัวเอง — และจะขยับ `resolution_id` คือ relabel
ข้าม index 40+ ตัวกับ result ~24k ไฟล์ เพื่อแลกกับพจนานุกรมหนึ่งรายการ ·
**จึงไม่มีการแก้ข้อมูลราคาถูกที่ลบแรงจูงใจของกฎที่ถูกปฏิเสธใน § 9.3 ได้** ซึ่งเป็นเหตุผล
เพิ่มอีกข้อว่าทำไมมันควรถูกปฏิเสธต่อไป
