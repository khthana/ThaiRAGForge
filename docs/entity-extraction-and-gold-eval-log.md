...
## 9. จับ "ชื่อสาขาเปล่าๆ" และยุบหลักสูตรที่ถูกเปลี่ยนชื่อปริญญา (20 ส.ค. 2569)

**ที่มา** — ไม่ได้มาจาก eval แต่มาจากการลอง `lexical_containment` (arm L′) ใน UI แล้วพิมพ์
`วิศวกรรมคอมพิวเตอร์` เปล่าๆ → ขึ้น `unmatched` พจนานุกรมคีย์ด้วยชื่อ canonical เต็ม
(`หลักสูตรวิศวกรรมศาสตรบัณฑิต สาขาวิชาวิศวกรรมคอมพิวเตอร์`) คนค้นหาพิมพ์ **ชื่อสาขา**
`match_programs` จึงไม่เจออะไร แล้ว arm L′ ก็ถอยกลับไปเป็น hybrid ธรรมดา **เงียบๆ**
บนคำถามรูปที่ระบบจริงเจอบ่อยที่สุด

### 9.1 `match_programs_by_field` — คลี่ชื่อสาขาเป็นทุกหลักสูตรที่เปิดสาขานั้น

คืน **ทุก** canonical ที่มีสาขานั้น ไม่เดาว่าหมายถึงระดับปริญญาไหน — การเดาคือความผิดพลาด
สลับระดับปริญญาที่ guard ของ `match_programs` เอง (11 ส.ค.,
`docs/program-matcher-absorption.md`) ถูกสร้างมาเพื่อกันพอดี · จับแบบ standalone phrase
เพื่อไม่ให้ `แมคคาทรอนิกส์` ไปยิงข้างใน `วิศวกรรมแมคคาทรอนิกส์` แล้วปั้นหลักสูตรที่สองขึ้นมา
สาขาที่ยาวที่สุดชนะเมื่อหลายตัวแมตช์ที่ตำแหน่งเดียวกัน

ต่อเข้าสามที่ **แยกขอบเขตกันตั้งใจ**:

| จุด | ค่า | เหตุผล |
| --- | --- | --- |
| `detect_entities(include_field_matches=)` | **ปิด** | ป้อน `entity_lookup`/`EntityFilter` ซึ่งตัวเลขที่ตีพิมพ์วัดมาโดยไม่มีมัน |
| `LexicalContainmentRetriever` | **เปิด** | คือจุดที่ฟีเจอร์นี้มีไว้เพื่อมัน |
| `classify_query` | branch **ท้ายสุด** ก่อน `ROUTE_UNMATCHED` | ดู 9.3 |

### 9.2 `programme_groups` — 253 → 250 และตัวแยกแยะต้องใช้สองสัญญาณ

`วิศวกรรมคอมพิวเตอร์` ชี้ไป 4 entry แต่เป็น 3 หลักสูตรจริง: หลักสูตรอนุปริญญาของโคเซ็นถูก
**เปลี่ยนชื่อปริญญา** ปี 2568 (`หลักสูตรอนุปริญญา` → `หลักสูตรอนุปริญญาวิศวกรรมศาสตร์`) —
`2569/3` แก้ไข *"ฉบับปี พ.ศ. ๒๕๖๗"* ซึ่งคือหลักสูตรที่ `2567/2` อนุมัติใต้ชื่อเดิม

**"ปีไม่ทับกัน" อย่างเดียวไม่พอ** — flag ได้ 5 คู่ ซึ่ง **3 คู่ผิด** เพราะปริญญาโทกับปริญญาเอก
ในสาขาเดียวกันก็แค่ไม่บังเอิญปรากฏพร้อมกันในหน้าต่าง 6 ปี · การเปลี่ยนชื่อจึงต้องมีเงื่อนไข
ที่สองว่า **ชื่อปริญญาหนึ่งคืออีกชื่อที่ถูกต่อให้ยาวขึ้น** และจำกัดไว้ที่ตระกูลอนุปริญญา
(`บัณฑิต` เป็นส่วนท้ายของ `มหาบัณฑิต` ถ้าไม่จำกัดก็จะยุบข้ามระดับปริญญา)

### 9.3 กฎที่ถูก**ปฏิเสธ** — และต้องถูกปฏิเสธต่อไป

กฎ "ปริญญาเดียวกัน สาขาหนึ่งขยายอีกสาขา" *ดู* สมมาตรกับ 9.2 แต่ไม่ใช่ · มันยุบไป
**28 entry** รวม `วิศวกรรมไฟฟ้า` กับ `วิศวกรรมไฟฟ้าสื่อสารและเครือข่าย` และ `ภาษาญี่ปุ่น`
กับ `ภาษาญี่ปุ่นธุรกิจ` — **ชื่อสาขาที่ยาวกว่าตามปกติคือคนละหลักสูตร** ซึ่งเป็นปัญหา
prefix-group ที่ docstring ของ `program_loader` เปิดหัวไว้เอง

- **จับได้เพราะอ่านกลุ่มที่ยุบ ไม่ใช่อ่านตัวเลข** — 28 ดูเหมือนกฎที่ดีกว่ากำลังทำงานได้มากกว่า
- เคสจริงเคสเดียวที่กฎนี้ถูกเขียนมาเพื่อมันคือ manifest ปี 2566 ใบเดียวที่พิมพ์ตก `วิศวกรรม`
  ออกจาก `สาขาวิชาวิศวกรรมแมคคาทรอนิกส์` — คือ *คำผิด* ราคา 1 entry (`count=1`)
- ผลจึงเป็น **7 ช่อง → 4 ไม่ใช่ 7 → 3** ตามที่สั่ง และบอกส่วนต่างตรงๆ
- `tests/test_program_field_matching.py` ตรึงทุก negative ไว้ และถูก **ตรวจว่าพังจริง
  เมื่อเอากฎกลับมา** ก่อนจะเชื่อ (พังพอดี 2 เทสต์ที่เขียนมาห้ามมัน)

### 9.4 ตำแหน่งของ branch เป็นเรื่องที่ **วัด** ไม่ใช่เลือก

- **0 จาก 106** คำถาม Gold เดินมาถึง branch นี้ → เลื่อนตัวเลข routing ที่ตีพิมพ์ไม่ได้
  โดยโครงสร้าง
- **5 จาก 13** คำถาม `faculty` มีชื่อสาขาอยู่ข้างในชื่อคณะ (`คณะบริหารธุรกิจ` มี
  `บริหารธุรกิจ`) → ถ้าวางไว้เหนือ `match_faculties` มันจะแย่งไป

**ข้อบกพร่องที่ 5 ข้อนี้จับได้อีกชั้น**: ตอนแรก fallback ใน `detect_entities` ผูกกับเงื่อนไข
"ไม่เจอหลักสูตร" และ docstring ของ retriever เขียนไว้แล้วว่า "ไม่เปลี่ยนอะไรเลยบน 106 Gold
queries" — **เท็จ** มันยิงใส่ 5 ข้อนั้น ขยายคำถามที่ resolve ได้อยู่แล้ว และเลื่อนตัวเลขของ
arm ที่ตีพิมพ์แบบเงียบๆ · แก้เป็นยิงเฉพาะตอนคำถาม resolve ไม่ได้ **เลยสักชนิด** ซึ่งทำให้
ประโยคนั้นเป็นจริง **โดยโครงสร้าง** (ทั้ง 106 ข้อ detect อะไรได้) ตรึงไว้ทั้งสองทิศทาง

### 9.5 Gold set วัดเรื่องนี้ไม่ได้เชิงโครงสร้าง

คำถาม `program` ทั้ง **30 ข้อเขียนชื่อ canonical เต็ม** — รูปคำถามที่ฟีเจอร์นี้รับใช้*ไม่มีอยู่*
ในชุดประเมิน จึงเป็น **deployment fix** และ **ห้ามอ้างตัวเลข retrieval ให้มันทั้งสองทาง**
ต่างจาก § 6/6b ที่การแก้ matcher ขยับ tag แล้วขยับ eval ได้จริง

และเพราะไม่มีรายงานไหนออกตัวเลขเหล่านี้ `tests/test_program_field_matching.py` จึง **เป็น
แหล่งที่มา** ของมัน (253→250, คู่โคเซ็น 3 คู่, 0-จาก-106, 5-จาก-13) คำนวณใหม่ทุกครั้งที่รัน
`pytest` แทนที่จะคัดลอกลง snapshot report ที่จะเน่าทีหลัง

**ไม่กระทบ tag/qrels** และเป็นการรัน ไม่ใช่การให้เหตุผล: `program_loader.py` เป็น 143 บรรทัด
เพิ่ม / **0 บรรทัดลบ** ใน hunk เดียวที่ต่อท้าย (`match_programs` จึงเหมือนเดิมทุกไบต์) และรัน
`match_programs` บนไฟล์คลังสุ่ม 400 ไฟล์เทียบกับ `f78ed5e` ได้ **0 จาก 400** ที่ต่างกัน ·
`classify_query` และ `detect_entities` อย่างละ **0 จาก 106** · ไม่ต้อง rebuild
`entity_tags_full` และไม่ต้อง regenerate `programs_by_file.json`
