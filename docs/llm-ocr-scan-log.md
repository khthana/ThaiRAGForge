...
1. **Phase 3 (เขียนกลับเข้าคอร์ปัสจริง) ยังไม่ได้เขียนโค้ดเลย** — ต้องตกลง
   backup convention ก่อน (น่าจะ `.pre_reocr.bak` ตามธรรมเนียมเดิมของ repo)
   และ policy สำหรับ 31 หน้าที่ไม่ clean (ดูตารางข้อ 2)
2. 31 หน้าใน review UI ใหม่ — ยังไม่มีใครกดตัดสินจริง
3. 9 ไฟล์ (6 เอกสาร) unresolved — ต้องเปิด PDF ต้นทางด้วยมือหาให้เจอ
4. `tools/corpus_prep/llm_ocr_scan.py` ยัง **untracked ใน git** ทั้งที่โค้ด
   ที่ commit ไปแล้วต้อง import ไฟล์นี้ — ถามผู้ใช้ไปแล้ว 2 รอบ ยังไม่ตัดสินใจ
